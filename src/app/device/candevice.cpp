...
int CanDevice::initCan(){
    ...
    */
    vic.AccCode=((uint_fast32_t)0x123)<<21;
    /*
    ...
    */
    vic.Filter=this->m_filterMode.toInt();
    /*
    ...
    while(m_exitStateThread1){
        ...
        if(lRel > 0){ /* 数据处理 */
            ...
            for(int i=0;i<lRel;i++){
                s_sendmore(publisher,string("CANRecv "));
                s_sendmore(publisher,string("CANTest "));
                string data;
                data.clear();
                char temp[3];
                for(int j=0;j<vco[i].DataLen;j++){
                    sprintf(temp,"%02X",vco[i].Data[j]);
                    data.append(temp);
                    data.append(" ");
                };
//                cout<<"data:"<<data;
                zmq::message_t message(100);
                snprintf((char *)message.data(),100,"%08X %02d %s %08X",vco[i].ID,data,vco[i].TimeStamp);
...
//    qDebug()<<"CanDevice::stateThread():"<<QThread::currentThread();
    ...
    const char *filter = "CANControl";//s
...
//    publisher= zmq::context_t(context, ZMQ_PUB);
