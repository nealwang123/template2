...
frmDbDelegate::frmDbDelegate(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::frmDbDelegate)
{
    ui->setupUi(this);
    createConnection();
    this->initForm();
...
*
*******************/
 bool frmDbDelegate:: createConnection()
{
...
//        ret=_db.open();
    ...
    if(ret == false){
        qDebug() << "delegate连接数据失败！";
        return false;
    }
    qDebug() << "delegate连接数据库成功";
    return true;
...
//    columnNames << "端口编号" << "端口名称" << "连接类型" << "通讯方式" << "串口号" << "波特率" << "IP地址" << "通讯端口" << "采集周期(秒)" << "通讯超时(次)";
    columnNames << "SID_NUM" << "NAME" << "StartBit" << "Length" << "Factor" << "Offset" << "nMax" << "nMin" << "isSigned" ;
    columnWidths.clear();
    columnWidths << 60 << 150 << 180 << 80 << 80 << 80 << 130 << 80 << 100 ;
