...
CLicCreateDlg::CLicCreateDlg(QWidget *parent, Qt::WindowFlags f)
    : QDialog(parent, f)
{
    setupUi(this);
...
void CLicCreateDlg::accept()
{
    if (tabWidget->currentIndex() == 0) {
        if (radioButton_permanent->isChecked()) {
            ...
            if (lineEdit_hostid->text().isEmpty() || lineEdit_hostid->text() == "" ||
                    lineEdit_hostid->text().trimmed().length() != NR_HOSTID_LEN) {
                strText += tr("Error Host ID!");
            }

            if (comboBox_softname->currentText().trimmed().isEmpty() ||
                    comboBox_softname->currentText().trimmed() == "") {
...
//              strText += " "+tr("contact ID");
        ...
        if (radioButton_permanent->isChecked()) {
            ...
            if (CreateLicense(strSoftName.toLocal8Bit().data(),
                              strHostID.toLocal8Bit().data(), strLicense) == NR_OK)
#endif
            {
            ...
            if (createTempLicense(strSoftName.toAscii().data(),
                                  nYear, nMonth, nDay, strLicense) == NR_OK)
#else
        ...
        if (radioButton_permanent_old->isChecked()) {
            if (lineEdit_hostid_old->text().isEmpty() || lineEdit_hostid_old->text() == "" ||
                    lineEdit_hostid_old->text().trimmed().length() != NR_HOSTID_LEN) {
                QPalette pl = label_status->palette();
                pl.setColor(QPalette::WindowText, QColor(Qt::red));
